...
@new_directory
def alignments_to_sdf(results: list,
                      mol_alignments: dict,
                      query_name: str,
                      num_of_mols: int = 50
                      ) -> None:
...
@new_directory
def mols_to_image(results: list,
                  query_name: str,
                  num_of_mols: int = 50):
    """
    ...
    keys = [f'{result[0]}\n\nTotal Score: {result[1]:.2f}' for result in results[:num_of_mols]]
    img = Draw.MolsToGridImage(mols_to_draw, subImgSize=(300,250), molsPerRow=5, legends=keys, returnPNG=False)
